...
import { categoriaFestaFilterCols, CategoriaFestaFilterType } from "./columns";
import { CategoriaFestaPageStates } from "./modal/types";

interface DataTableProps {
  columns: ColumnDef<CategoriaFesta>[];
  data: CategoriaFesta[];
  pagination: PaginationTable;
  setPagination: (pagination: PaginationTable) => void;
  changeFilters: (name: string, value: string | SortingType | undefined) => void;
  filters: CategoriaFestaFilterType;
  clearFilters: () => void;
  isLoading: boolean;
  showState: CategoriaFestaPageStates;
  changeShowState: (name: keyof CategoriaFestaPageStates, value: boolean) => void;
  removeCategoriaFesta: () => void;
  isDeleting: boolean;
}

export function CategoriaFestaDataTable({
  columns,
  data,
  pagination,
  setPagination,
    ...
    <>
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center">
                          {Object.keys(categoriaFestaFilterCols).includes(header.id) ? (
                            <>
                              {categoriaFestaFilterCols[
                                header.id as keyof typeof categoriaFestaFilterCols
                              ].sortable ? (
                                <SortTable
                                  filters={filters}
                                  name={
      ...
      {showState.showDialog ? (
        <DeleteConfirmationDialog
          ...
          onConfirm={removeCategoriaFesta}
